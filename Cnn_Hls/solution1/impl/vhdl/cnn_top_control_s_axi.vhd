...
-- 0x1c : reserved
-- 0x20 : Data signal of kernel_V
--        bit 31~0 - kernel_V[31:0] (Read/Write)
-- 0x24 : reserved
...
architecture behave of cnn_top_control_s_axi is
    ...
    constant ADDR_KERNEL_V_DATA_0 : INTEGER := 16#20#;
    constant ADDR_KERNEL_V_CTRL   : INTEGER := 16#24#;
    ...
    signal int_kernel_V        : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_KERNEL_V_DATA_0 =>
                        rdata_data <= RESIZE(int_kernel_V(31 downto 0), 32);
                    when others =>
                        rdata_data <= (others => '0');
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    kernel_V             <= STD_LOGIC_VECTOR(int_kernel_V);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_KERNEL_V_DATA_0) then
                    int_kernel_V(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_kernel_V(31 downto 0));
                end if;
            end if;
        end if;
    end process;
